...
class ImbalancedData(): 
    def __init__(self, n_items = 5000, classes=[8, 0], proportion=0.9, n_val=5, random_seed=1, mode="train"):
        if mode == "train":
...
def get_mnist_loader(batch_size, classes=[8, 0], n_items=5000, proportion=0.9, n_val=5, mode='train'):

    dataset = ImbalancedData(classes=classes, n_items=n_items, proportion=proportion, n_val=n_val,mode=mode)
